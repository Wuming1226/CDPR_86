...
class Motor:
    def __init__(self, can_channel, can_id):
    ...
    def set_pdo(self):
        ...
        # msg = can.Message(arbitration_id=self.req_address,
        #                   data=[0x2F, 0x00, 0x1A, 0x00, 0x00, 0x00, 0x00, 0x00], is_extended_id=False)
        # self.safe_send_can_msg(msg)
        # self.show_sdo_response()
        # time.sleep(0.002)
        # # set mapping object: motor position 606400 32bit
        # msg = can.Message(arbitration_id=self.req_address,
        #                   data=[0x23, 0x00, 0x1A, 0x01, 0x10, 0x00, 0x64, 0x60], is_extended_id=False)
        # self.safe_send_can_msg(msg)
        # self.show_sdo_response()
        # time.sleep(0.002)
        # # set mapping number
        # msg = can.Message(arbitration_id=self.req_address,
        #                   data=[0x2F, 0x00, 0x1A, 0x00, 0x02, 0x00, 0x00, 0x00], is_extended_id=False)
        # self.safe_send_can_msg(msg)
        # self.show_sdo_response()
        # time.sleep(0.002)
    ...
    def get_current_position_command(self):
        msg = can.Message(arbitration_id=self.req_address, data=[0x40, 0x7A, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00],
                          is_extended_id=False)
        self.safe_send_can_msg(msg)
        time.sleep(0.002)
    ...
    def set_target_position_abs(self, target_position=0):
        target_position = round(target_position * self.encoder_resolution / (2 * np.pi))
        data = int_to_le_bytes(target_position)
        ...
        #    if msg is not None and msg.arbitration_id == self.ans_address:
        #        #print(msg)
        #        if msg.data[1] == 0x7A and msg.data[2] == 0x60:
        #            if msg.data[0] == 0x60:
        #                print('set success')
        #                print(self.get_current_position_command())
        #                #print(msg)
    ...
    def set_target_position_rel(self, target_position=0):
        target_position = round(target_position * self.encoder_resolution / (2 * np.pi))
        data = int_to_le_bytes(target_position)
        # msg = can.Message(arbitration_id=0x300 + self.can_id,
        #                   data=[data[0], data[1], data[2], data[3], 0x00, 0x00, 0x00, 0x00], is_extended_id=False)
...
if __name__ == "__main__":
    ...
    _parser = argparse.ArgumentParser(
        description="Motor position test script: optional motor_id and pos_movement (replaces 0.1 in set_target_position_abs)."
    )
    _parser.add_argument(
        ...
        type=int,
        default=3,
        help="CAN node id for Motor (default: 3)",
    )
    _parser.add_argument(
        "pos_movement",
        nargs="?",
        type=float,
        default=0.1,
        help="Coefficient of pi in target: pos_movement * pi + init offset (default: 0.1)",
    )
    _args = _parser.parse_args()
    motor_id = _args.motor_id
    pos_movement = _args.pos_movement
    ...
    motor1.set_profile_position_mode(velocity=2)
    print(motor1.get_current_position_command())
    init_pos = motor1.get_current_position()
    print(init_pos)
    motor1.set_target_position_abs(pos_movement * np.pi + init_pos / 10000 * 2 * np.pi)
    time.sleep(2)
    #motor1.set_target_position_abs(-0.1 * np.pi + init_pos / 10000 * 2 * np.pi)
    #time.sleep(2)
    print(motor1.get_current_position_command())
    #motor1.shut_down()
    print('end')
