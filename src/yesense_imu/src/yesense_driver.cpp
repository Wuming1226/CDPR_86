...
// 功能模式查询 0x4D (子类型0x02: 算法模式 AHRS/VRU/IMU)
...
// 功能模式设置 0x4D
// 低4位: 1=AHRS, 2=VRU, 3=IMU (子类型0x02); 0x0A=陀螺零偏初始化 (子类型0x50)
// bit7=1: RAM(掉电不保存), bit7=0: Flash(掉电保存)
void YesenseDriver::onModeSettingSetting(const std_msgs::UInt8::ConstPtr& msg)
{
    boost::shared_array<uint8_t> buffer = boost::shared_array<uint8_t>(new uint8_t[64]);

    uint8_t mode = msg->data & 0x0f;
    if ((mode < 1 || mode > 3) && mode != 0x0A)
        return;
...
void YesenseDriver::publish_imu(const protocol_info_t &imu_data)
{
    ...
    inertial_data.magnetic.x = imu_data.mag_x;
    inertial_data.magnetic.y = imu_data.mag_y;
    inertial_data.magnetic.z = imu_data.mag_z;
    inertial_data.raw_magnetic.x = imu_data.raw_mag_x;
    inertial_data.raw_magnetic.y = imu_data.raw_mag_y;
    inertial_data.raw_magnetic.z = imu_data.raw_mag_z;
